...
function Project({ index, total, progress }) {
  ...
  //highlight amount
  const highlight = Math.min(1,Math.max(0, 
    1 - Math.abs(rotation / HIGHLIGHT_THRESHOLD)
  ));

  //styles
  const rotationTransform = {transform: `rotate(${rotation}deg) translateX(15vw)`};
  ...
  return (
    <div className={`${styles.container} ${isSelected ? styles.selected : ""}`} style={rotationTransform}>
      <div className={styles.img}>
        <img className="img-container" src="/public/projects/SotF.png" style={imgOpacity} />
      </div>
